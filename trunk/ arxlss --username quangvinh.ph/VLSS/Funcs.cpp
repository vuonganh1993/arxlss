...
{
	CLogger::Print(L"*Call: findDocument(%s)", pszFileName);
	AcApDocument* pCurrDoc = acDocManager->curDocument();

	if (pCurrDoc) {
		LPCASTR pszTmp = pCurrDoc->fileName();
		if (0 == lstrcmpi(pszFileName, pszTmp)) {
			CLogger::Print(L"*Exit: findDocument() - Found out!");
			return pCurrDoc;
		}
	}
	...
	delete pDocIter;
	CLogger::Print(L"*Exit: findDocument() - Have not found out.");
	return NULL;
}
